...
class QuellCache {
  ...
  async query(req, res, next, isQuellable = true) {
    ...
    if (proto === 'unQuellable' || !isQuellable) {
      ...
      const protoMutate = JSON.parse(JSON.stringify(protoForCache));
      ...
      if (Object.keys(queryObject).length > 0) {
        ...
        graphql(this.schema, newQueryString)
          .then(async (queryResponse) => {
            ...
            // //console.log("here is TORETURN======>>>>>", toReturn);
    ...
    for (const superField in proto) {
      ...
      if (isCollection) {
        ...
        if (!collection) {
          ...
          const collectionFromCache = await this.getFromRedis(
            identifierForRedis
          );
          //console.log(
   ...
   */
  async joinResponses(cachedData, uncachedData, protoObj, queryMap) {
    console.log('this.queryMap', queryMap);
    // data is always is object
    //console.log('we are in joinResponses', uncachedData, cachedData, protoObj);
    ...
    for (const key in uncachedData) {
      console.log('KEY IS', key); //, Array.isArray(protoObj[key]));
      // if we have an array run initial logic for array
      if (Array.isArray(queryMap[key])) {
        console.log('key in protoObj', key);
        joinedData[key] = [];
        let dataToIterate; //= cachedData[key] || uncachedData[key];
        console.log('dataToIterate before -->', dataToIterate);
        if (uncachedData.hasOwnProperty(key)) {
          dataToIterate = uncachedData[key];
          console.log('===================uncached=======================');
        }
        if (cachedData.hasOwnProperty(key)) {
          console.log('cached');
          dataToIterate = cachedData[key];
        }
        console.log('dataToIterate -----> after', dataToIterate);
        for (let i = 0; i < dataToIterate.length; i += 1) {
          console.log('===================hapennnnnn', uncachedData[key][0]);
  ...
  async constructResponse(fullResponse, AST) {
    ...
    //console.log('rebuild from ast', rebuiltProto);
