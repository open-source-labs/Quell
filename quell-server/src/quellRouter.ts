...
interface QuellRouterOptions {
  ...
  headers?: {
    // Default headers for each API
    [apiName: string]: {
      // API name as the key (derived from endpoint path)
  ...
  const routerMiddleware = async (
    ...
    try {
      ...
      if (!targetUrl || targetUrl === "local") {
        if (debug)
          console.log(
            `QUELL ROUTER: Path ${path} not found or marked as local, passing to next middleware`
          );
        return next();
      }
      ...
      if (!query) {
        if (debug)
          console.log(
            "QUELL ROUTER: No query provided, passing to next middleware"
          );
        return next();
      }
      ...
      if (debug)
        console.log(
          `QUELL ROUTER: Cache miss for ${apiName}, fetching from API at ${targetUrl}`
        );
    ...
    const responseToCache = { ...apiResponse };
    delete responseToCache.cacheHit;
          ...
          if (debug)
            console.log(
              `QUELL ROUTER: Cached response for ${apiName} (TTL: ${cacheExpiration}s)`
            );
        } catch (cacheError) {
      ...
      return res.status(500).json({
        errors: [
          {
            message: `Error in GraphQL router: ${
  ...
  routerMiddleware.clearApiCache = async (apiName: string): Promise<number> => {
    try {
      ...
      if (keys.length > 0) {
        ...
        if (debug)
          console.log(
            `QUELL ROUTER: Cleared ${keys.length} cached responses for ${apiName}`
          );
        return keys.length;
      }
