...
const DemoInput = () => {
  ...
  const outputFunction = (newList, sub, query) => {
    const newOutput = ResultsHelper(newList, sub, query, output)
    setOutput[newOutput]
  };
  ...
  const selectDropdownId = (item) => { // item comes in as number (2)
    setSelectedId(item)
  ...
  const dropdownList = [
    'countries', 'country by id', 'cities', 'city by id'
  ];
    ...
    <>
      ...
      <div className="queryLine">{tab} 
        ...
        {query}
          ...
          {idDropdown && <button className="dropdown-button" 
            onClick={() => toggleIdDropdownMenu(!idDropdownMenu)}
          >▾{idDropdown && selectedId}</button> }
        {ob}
      </div>
      ...
      <div className="queryLine">{tab} {cb}</div>
